...
public class PhotoBoardServiceImpl implements PhotoBoardService {
  ...
  final static Logger logger = LogManager.getLogger(PhotoBoardServiceImpl.class);
  ...
  public int add(PhotoBoard board) {
    ...
    return fileDao.insert(board.getPhotoFiles());
  }
  ...
  public int update(PhotoBoard board) {
    ...
    // 트랜잭션 동작방식을 설정한다.
    // PhotoBoard 객체에 제목이 들어있다는 것은 사용자가 변경했다는 뜻이다.
    if (board.getTitle() != null) {
      boardDao.update(board);
    }

    List<PhotoFile> photoFiles = board.getPhotoFiles();
    if (photoFiles != null) {
      // 먼저 기존 첨부 파일을 삭제한다.

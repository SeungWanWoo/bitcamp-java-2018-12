...
public class Servlet03 extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response) 
     ...
     * HTTP/1.1 302
        ...
        Content-Length: 0
        Date: Tue, 02 Apr 2019 03:38:52 GMT
        빈줄
        <----콘텐트를 보내지 않는다.
     */
    ...
    // HTML을 출력하는 경우
    // 응답헤더가 아니라 HTML 헤더에 리프래시 명령을 설정할 수 있다.
    out.println("<title>리다이렉트</title>");
    out.println("</head><body>");
    // 버퍼가 꽉차서 클라이언트로 자동으로 출력되어 리다이렉트가 안 먹히는 상황을 보고 싶다면
    //  다음 반복문 주석을 풀라!
//    for (int i = 0; i < 350; i++) {
...
//    }
    ...
    // 클라이언트에게 URL을 알려줄 때, 상대 경로로 지정해 줄 수 있다.

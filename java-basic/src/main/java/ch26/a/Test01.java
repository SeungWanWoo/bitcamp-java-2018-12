...
public class Test01 {
  public static void main(String[] args) throws Exception {
    ...
    //    예)c:/Users/user/git/bitcamp-2018-12/java-basic/src/main/java/ch26/a/mybatis.xml
    ...
    // 대충 봐도 경로를 설정하는 부분에서 불편함이 느껴진다.
    //
//    InputStream inpuStream2 = new FileInputStream(
//        "/Users/user/git/bitcamp-2018-12/java-basic/src/main/java/ch26/a/mybatis-config.xml");
    ...
    //       그 일을 대신 해주는 객체 '빌더'를 통해 생성하는 것이 유지보수에 좋다.)
    //
    // => SqlSessionFactory 
    //    - SqlSession 객체를 만들어 주는 일.
    //    - Factory Method 디자인 패턴이 적용되어 있다.
    //      (객체 생성 과정이 복잡할 경우 메서드를 통해 객체를 생성하는 것이 유지 보수에 좋다.)
    //
    // => SqlSession : SQL Mapper 파일에 보관된 SQL을 찾아 실행해주는 일.
    SqlSessionFactory sqlSessionFactory = 
        new SqlSessionFactoryBuilder().build(inputStream);
    
    // SqlSession sqlSession = new SqlSession(); // 이런 식으로 하면 관리하기 힘들다.
    // 3) 팩토리 메서드를 통해 객체를 생성한다.
    SqlSession sqlSession = sqlSessionFactory.openSession();
    
    // 4) SQL Mapper 파일에 보관된 SQL 문을 찾아 실행한다.
    // => 파라미터 값은 SQL 문의 id 이다.
    // => SQL Mapper 파일의 namespace 값과 SQL id 값을 결합해서 지정한다.
    // => selectList의 리턴 값은 SQL Mapper 파일의 resultType에 지정된 객체를 담고 있는
    //    List 객체이다.

...
public class RemoteObjectServer {
  public static void main(String[] args) {
    ...
    //    > rmiregistry
    // => 주의! RMI Registry 서버가 클래스를 찾을 수 있도록 .class 파일이 있는
    //    디렉토리에서 실행하라.
...
//    }
    ...
    try {
      ...
      // 3) 원격 객체와 통신을 담당할 프록시(클라이언트 측 OBR)를 생성한다.
      // => 첫 번째 파라미터: 원격 객체
      // => 두 번째 파라미터: 포트 번호. 0으로 지정하면 임의의 유효한 포트 번호가 지정됨
      ...
      // 4) RMI Registry 서버와 다룰 도구를 준비한다.
      Registry registry = LocateRegistry.getRegistry("study.bitcamp.co.kr");
      
      // 5) RMI Registry 서버에 stub 객체를 등록해줄 도구를 준비한다.
      // => rebind() : 해당 이름으로 stub 객체를 등록한다.
      //               만약 그 이름으로 등록된 스텁이 있다면 기존에 등록된 stub 객체를
      //               대체한다.
      // => bind() : 해당 이름으로 스텁 객체를 등록한다.
      //             만약 그 이름으로 등록된 스텁이 있다면 에러가 발생한다.
      registry.rebind("calc", stub);
      
      System.out.println("Calculator 객체의 프록시(stub)가"
          + " RMI Registry 서버에 등록되었다."); 
    } catch (Exception e) {
      e.printStackTrace();
    }
  }

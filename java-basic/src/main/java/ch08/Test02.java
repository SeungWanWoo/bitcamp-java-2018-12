...
public class Test02 {
  public static void main(String[] args) {
    ...
    // 3) main() 메서드에 선언된 로컬 변수(args)를 "JVM 스택 영역"에 생성한다.
...
/*
 *  # 클래스 로딩
 *   - 외부 저장장치(예: HDD, USB 메모리, DVD-ROM 등)에 있는 .class 파일을 JVM이
 *     관리하는 메모리로 로딩하는 것을 말한다.
 *  - JVM이 관리하는 메모리 영역
 *     1) Heap
 ...
 *          - 스태틱 필드를 이 영역에 생성한다.
 *          - 사용하려는 명령어를 올려놓는 장소
 *          - 프로그램을 실행하기 위해서는 Method Area에 명령어가 올라와 있어야 한다.
 *          - 
 * # 클래스 로딩
 *  - 클래스의 코드를 사용하는 시점에 메모리(Method Area 영역)에 로딩된다.
 *  - 한 번 로딩되면 JVM을 종료할 때까지 유지한다.
 *  - 강제로 unloading 할 수 있다. 그리고 다시 로딩할 수 있다.[일반 개발자 사용 X]
 * # 클래스의 코드를 사용하는 시점?
 *  - 바로 스태틱 멤버(필드와 메서드)를 사용할 때
 *    예) My2.member = 200;
 *    예) System.out.println(My2.member);
 *    예) Integer.parseInt(..); => 스태틱메서드.클래스메서드(..);

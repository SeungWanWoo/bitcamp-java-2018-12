...
public class Test07 {
  public static void main(String[] args) {
    ...
    // => 코드의 값은 0 ~ 65535 이다.
    // => 음수가 없다.[문자 코드니까 음수가 존재할 수 없다.]
    // => 참고! 숫자 2바이트의 범위는 -32768 ~ 32767 이다.[문자하고 다르다는 점!]
    System.out.println((char)0xac00); //UTF-16 코드 값을 지정한다.
    
    // 문자의 코드를 알지 못한다면 작은 따옴표(' ')를 이용할 수 있다.
    // '가'?
    // 한글 '가'의 유니코드(UTF-16) 값을 리턴한다.
    // 또한 이 값이 문자의 코드 값임을 알려준다.
    System.out.println('가');
    
    System.out.println('가' + 2);    //숫자가 출력된다.
    System.out.println((char)('가' + 2));    //해당 코드 값에 대한 문자가 출력된다.
 ...
 * 1) ASCII (7bit)
 *  - 미국 국가 표준 협회(ANSI)에서 영어 알파벳, 숫자, 특수 기호(문자) 등을 컴퓨터에서 저장할 수 있도록
 *    7비트 2진수로 표현할 수 있는 정의한 인코딩 규칙 https://ko.wikipedia.org/wiki/ASCII
 *  - A(0x41), B(0x42), a(0x61), b(0x62), 스페이스(0x20), LF(0x0a), CR(0x0d),  
 *  !(0x21), #(0x23), $(0x24), %(0x25), &(0x26), +(0x2B), 
 *  0(0x30), 1(0x31), =(0x3d), ?(0x3f), @(0x40)
 *  - 참고
 *    윈도우 
 *    OS에서는 LFCR 두 바이트를 사용하여 줄 바꿈을 표시한다
 ...
 *     노르웨이어, 덴마크어, 독일어, 로만슈어, 스웨덴어 스코틀랜드 게일어, 아이슬란드어, 
 *     아일랜드어, 에스파냐어, 영어, 이탈리아어, 페로어, 포르투갈어, 프랑스어(일부[* 2]), 
 ...
 *   - 국제 표준이다.
 *   - 한글 음절 한 개에 대한 인코딩 규칙을 정의한 완성형 인코딩 규칙이다.
 *   - 현재 유통되는 모든 한글 음절을 표현할 수 없다.
 *   - 가(0xb0a1), 각(0xb0a2), 똘(0xb6ca), 똠(?), 똥(0xb6cb) 등
 * 4) 조합형(2바이트; 초성(5비트) + 중성(5비트) + 종성(5비트))
 *   - 국제 표준이 아니다.
 *   - 초성 : ㄱ(00010), ㄲ(00011), ㄴ(00100), ㄷ(00101), ...
 *   - 중성 : ㅏ(00011), ㅒ(00100), ㅑ(00101), ㅒ(00110), ...
 ...
 *  - 국제 표준이 아니다.
 *  - 윈도우 OS의 인코딩 규칙이다.
 *  - EUC-KR + 현재 유통 되는 한글 음절 추가 = MS949 (11172자 + 한국 전용 한자 + 옛 한글 등)
 *  - MS949에 추가된 문자
 *     똠(0x8c63), 똡(0x8c64) 등
 *  - 기존의 EUC-KR에 문자를 추가했기 때문에 정렬할 때 문제가 발생한다.
 *    "똘똠똡똥(b6ca 8c63 8c64 b6cb)" 문자를 정령한다면,
 ...
 *  - Unicode(UTF-16)을 그대로 사용하면, 기존의 OS를 모두 변경해야 한다.
 ...
 *  - 규칙
 *    0000 ~ 007f(7bit ASCII) : 이전과 똑같이 1바이트로 표현한다.
 ...
 *  - 한글은(ac00 ~ d7af) 0800 ~ ffff에 속하기 때문에 3바이트로 변환한다.
 *  - UTF-16 '가(ac00)'를 UTF-8로 인코딩하기
 *     '가' => 1010 1100 0000 0000 (UTF-16)
 *         => 11101010 10110000 10000000 (UTF-8)
 *         => 0xEAB080 
 */     

...
// => CLASS 
//    - .class 파일까지는 유지된다. 
//    - runtime에는 메모리에 로딩되지 않는다.
//    - 애노테이션을 정의할 때 유지 범위를 지정하지 않으면 기본이 CLASS이다.
// => SOURCE
//    - 컴파일할 때 제거된다. 
//    - .class 파일에 포함되지 않는다.
//    - 보통 소스 파일에서 애노테이션 값을 꺼내 다른파일을 자동 생성하는 도구를 만들 때
//      많이 사용한다.
// => RUNTIME 
//    - .class 파일까지 유지되고, runtime 메모리에 로딩된다.
...
@MyAnnotation // .class 파일에서 확인할 수 있다. 유지범위가 클래스이기 때문에 추출되지 않는다.
@MyAnnotation2 // .class 파일에 없다.
@MyAnnotation3 // .class 파일에서 확인할 수 있다.
public class Test01 {
  public static void main(String[] args) {
    // Test01.class를 편집기로 열어서 확인해보라
    ...
    // 그러나 MyAnnotation3는 유지범위가 RUNTIME이기 때문에 실행할 때 메모리에 로딩된다.

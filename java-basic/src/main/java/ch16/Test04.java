...
// => class 클래스명<타입 파라미터명, 타입 파라미터명, ...> {...}
// => 클래스가 다루는 어떤 타입이 있다고 가정하자. 그 타입을 T라고 부르겠다.

class Box2<T> {
  T value; // T 타입의 인스턴스 주소를 저장할 필드
...
public class Test04 {
  ...
  public static void main(String[] args) {
    ...
    Box1 b1 = new Box1(100); // = Integer.valueOf()의 오토박싱되서 들어감
    ...
    // => 값을 꺼낼 때마다 형변환 해야 한다.
    //
    int v1 = (int) b1.getValue(); // <= 리턴값.intValue(); 라는 메서드 호출
    String v2 = (String) b1.getValue();
    Date v3 = (Date) b1.getValue();
    ...
    // new를 실행할 때는 굳이 또 타입을 지정할 필요가 없다. 생략해도 된다. <>는 냅둔다.
    Box2<String> x2 = new Box2<>("홍길동");
    ...
    // => 즉 형변환 할 필요가 없다.
    String y1 = x1.getValue(); // 형변환 안해도 된다.

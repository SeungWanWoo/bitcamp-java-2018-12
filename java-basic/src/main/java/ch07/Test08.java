...
public class Test08 {
  ...
  public static void main(String[] args) {
    ...
    // new int[3] : int 변수 3개 생성한다. 각 항목에 저장하는 값 int 값!
    ...
    // 이런 상태에서 레퍼런스 배열에 있는 항목을 사용하려 하면 NullPointerException 오류가
    // 발생한다.
    // System.out.println(scores[0].name); // 실행 오류! (Runtime exception 발생) 
    // scores 주소를 찾아가서 0번 인덱스의 값에
    // 대한 주소값을 참조하고 싶어도 null 값이기 때문에 NullPointerException 오류가 발생
    ...
    System.out.println(scores[2].aver);
    // 잊지말자!
    // - 인스턴스 배열을 만들 수 없다.
    // - 레퍼런스 배열만 가능하다.
    // - 단, 자바 기본 타입(byte, short, int, char, long, double, float, boolean) 은

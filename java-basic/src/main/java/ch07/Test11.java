...
public class Test11 {
  public static void main(String[] args) {
    ...
    //      import 패키지명.클래스명;
    //
    // 특정 패키지의 클래스를 많이 사용한다면 
    // 다음과 같이 패키지명 다음에 wildcard(*)를 지정할 수 있다.
    //
    //      import 패키지명.*;  
    //
    // => 이 경우 해당 패키지의 모든 클래스를 패키지명 없이 사용할 수 있다.
    // => 서브 패키지는 해당이 안된다.
    // => 단 소스 코드를 읽을 때 어떤 클래스가 어떤 패키지에 있는지 바로 확인할 수 없는
    //    불편함이 있다. 그래서 대부분의 자바 개발자들은 널리 알려진 클래스가 아닌 경우
    //    가능한 wildcard를 사용하지 않고 패키지명과 클래스명을 정확하게 명시한다.
    //
    // 단, import 명령은 package 명령 다음에 와야하며,
    // 클래스 선언 전에 와야 한다.
    ...
    //  예를 들어) java.lang.annotation, java.lang.invoke 등
    //  
  }
}

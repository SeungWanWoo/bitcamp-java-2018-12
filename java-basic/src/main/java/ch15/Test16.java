...
public class Test16 {
  ...
  // => Cloneable을 구현하는 이유는 JVM에게 이 클래스의 인스턴스를 복제할 수 있음을
  //     표시하기 위함이다. 이 표시가 안된 클래스는 JVM이 인스턴스를 복제해 주지 않는다.
  //
  // 인스턴스를 복제할 때는,
  // => Object에서 상속 받은 clone을 호출해야 한다.
  ...
  static class Score implements Cloneable {
    ...
    // => Object에서 상속 받은 clone()을 오버라이딩하여 다른 패키지의 멤버도
    //     사용할 수 있게 public으로 접근 범위를 넓혀라.
    // => 오버라이딩은 접근 범위를 좁힐 수는 없지만 넓힐수는 있다.
    // => 오버라이딩 할 때 리턴 타입을 클래스 타입으로 변경해도 된다.
    @Override
    public Score clone() throws CloneNotSupportedException{
      // 복제를 위한코드를 작성할 필요가 없다. JVM이 알아서 해준다.
      // 그냥 상속 받은 메서드를 호출하라!

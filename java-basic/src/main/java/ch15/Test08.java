...
public class Test08 {
  ...
  public static void main(String[] args) {
    ...
    // Map은 값을 저장할 때 키와 value 두 개를 사용한다.
    ...
    // put(Object key, Object value)
    // -> key 값으로 int를 넘겨준다. 내부적으로 auto-boxing을 수행하여 Integer 객체를
    //     만든다. 그리고 그 객체를 넘겨주는 것이다.
    // => put() 메서드는 key로 넘겨받은 객체에 대해 hashCode()를 호출하여
    //     정수 값을 얻는다.
    // => 해시 코드를 사용하여 저정할 위치를 계산한다.
    // => 그런 후 그 위치에 해당하는 배열(배열로 관리한다면)에 저장한다,
    ...
    map.put(100,  new Student("홍길동", 20, false));
    map.put(101,  new Student("임꺽정", 30, true));
    map.put(102,  new Student("유관순", 17, true));

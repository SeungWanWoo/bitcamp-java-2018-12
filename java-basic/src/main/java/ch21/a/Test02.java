...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    //System.out.println(sum(100000L)); // 실행 오류!
      for (int i = 0; i < 1000; i++) {
        list.add(new long[1000000]); // Heap 영역에 8MB 크기의 long 배열을 생성한다.
      ...
      // JVM에게 예외 상항이 보고되지 않으면 즉시 멈추지 않는다.
      // 
      // 시스템 예외인 경우 현재 애플리케이션 작업 결과를 저장하고 로그 파일에 예외 정보를
      // 남긴 후 프로그램을 종료하는 것이 좋다.
      //
    ...
    // 
    list.add(new long[1000000]); // 8메가 추가하는데에러가 뜸
    // ^ 메모리가 부족한 상태이기 때문에 실행 오류! 해결 불능!

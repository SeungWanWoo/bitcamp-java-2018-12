...
public class Test06 {
  public static void main(String[] args) {
    ...
    */
    ...
    //-----------------------^--배열의 주소값이지 배열 자체가 들어가는 것이 아니다.
    // 실행 순서
    // 1) int 배열 3개를 만든다.
    //  2) 중괄호{}에 들어 있는 값으로 각 배열 항목을 초기화시킨다.
    //  3) 배열의 주소를 파라미터의 값으로 놓는다. => plus(1237번지);
    // 주의 ! 배열 자체가 넘어 가는 것이 아니라, 배열의 주소가 넘어간다는 것이다.
    System.out.println(result); 
    
    // result = plus({100, 200, 300}); // 배열 레퍼런스가 있을 때는 new int[]를
    // 생략할 수 있지만 없는 경우는 생략할 수 없다. => 컴파일 오류!
  }
  ...
  static int plus(int... value) {
    ...
    //^ 메서드 안에 선언된 변수를 "로컬 변수(local variable)"이라 한다.
    // <== 파라미터도 로컬 변수입니까? 예, 로컬 변수가 맞다.

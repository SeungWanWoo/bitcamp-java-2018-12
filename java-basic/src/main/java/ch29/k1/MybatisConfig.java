...
// => @MapperSan(DAO 인터페이스가 들어 있는 패키지)
...
public class MybatisConfig {
  ...
  public SqlSessionFactory sqlSessionFactory(
    ...
    // "생성하는 객체 + FactoryBean" 이름으로 짓는다.
    // 그런데 Mybatis-spring 라이브러리에서는
    // SqlSessionFactoryFactoryBean이라는 이름으로 짓지 않고
    // 중간에 Factory가 두번 들어가서 보기 싫다고 한 개를 빼 버렸다.
    // 그래서 SqlSessionFactoryBean이라는 이름이 되었다.
    // 주의하라! 
    // 기존의 이름관행에 따라 SqlSessionFactoryBean이 SqlSession 객체를 생성한다고
    // 생각하기 쉬운데, 아니다 SqlSessionFactoryBean은 SqlSessionFactory 객체를
    // 생성해준다.
    //

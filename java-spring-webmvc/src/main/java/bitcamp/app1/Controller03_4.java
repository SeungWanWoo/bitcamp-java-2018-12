// request handler 구분하는 방법 - Content-type 헤더의 값에 따라 구분하기
...
@RequestMapping("/c03_4") // 컨트롤러에 URL을 매핑한다.
public class Controller03_4 {
  ...
  //    이 메서드를 호출하라는 의미다.
  //
  // 클라이언트가 POST로 요청할 때 보내는 유형에 따라 호출될 메서드를 구분할 수있다.
  ...
  //다음 메서드는 Content-Type 헤더가 없을때 출력된다.
  @RequestMapping
  @ResponseBody

...
// 1. Stateful 방식
//    - 서버와 연결한 후 연결을 끊을 때 까지 계속 통신하는 방식
//    - 클라이언트가 연결을 끊지 않으면 서버 쪽에 계속 소켓이 유지되기 때문에 메모리를
//      일정 점유한다.
...
//    - 단 요청할때마다 서버와 연결해야 하기 때문에 연결하는데 일정 시간이 소요된다.
//    - 그러나 보다 많은 클라이언트 요청을 처리할 수 있어 대부분의 서비스에서 이 방식을 많이 사용한다.
// 3. Stateless + 멀티 스레드 방식
//    - 특히 statless 방식에 멀티스레드를 적용하면 동시에 많은 클라이언트 요청을 처리할 수 있다.
//    - 대부분의 서비스들이 이 조합을 사용한다.
//
// 9 단계의 목표가 "Stateless + 싱글스레드" 방식을 "Stateless + 멀티 스레드"로 바꾸는 것이다.
// 작업:
// 1) 클라이언트 요청 처리를 전담할스레드 클래스를 정의한다.

...
public class DataSource {
  ...
  public DataSource(String driver, String jdbcUrl, String username, String passoword) {
    this.driver = driver; this.jdbcUrl = jdbcUrl;
    this.username = username; this.password = passoword;
  }
    ...
    try {
      ...
      if (con == null) { // 현재 스레드 주머니에 Connection 객체가 들어있지 않다면,
        if (list.size() == 0) { // ArrayList에도 Connection 객체가 없다면,
          System.out.println("새로 DB Connection 생성함!");
          con = DriverManager.getConnection( // 새로 DB Connection을 생성한다.
              "jdbc:mariadb://localhost/bitcampdb?user=bitcamp&password=1111");
          con.setAutoCommit(false);
        } else { // ArrayList에 Connection 객체가 있다면,
          System.out.println("ConnectionPool에 보관된 것을 리턴함!");
          con = list.remove(0); // ArrayList에서 Connection 객체를 꺼낸다
        }
        //일단 준비된 Connection 객체는 현재 스레드에서 사용할 수 있도록 주머니에 보관한다.
        conLocal.set(con);
      } else {
        System.out.println("스레드 주머니에 보관된 Connection 객체 리턴함");
      }
      return con;

...
//    DAO가 작업을 할 때 스레드에 보관된커넥션 객체를 이용한다.
//
// 작업:
// 1) ConnectionFactory 변경
//    => create()를 호출할 때 먼저 스레드에 저장된 것이 있는지 검사한 후
//       있으면 그 Connection 객체를 return하고,
//       없으면 새 Connection 객체를 만들어 return한다.
//    => commit()/rollback()을 사용하려면 커넥션의 auto_commit을 false로 설정해야한다.
// 2) AbstractCommand 변경
//    => execute()에 commit()과 rollback()을 적용한다.
// 3) Command 구현체 변경
//    => Command에서 Connection 객체를 사용한 후 닫지 않도록 한다.

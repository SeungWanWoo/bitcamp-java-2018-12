...
//    - 왜? 클라이언트 1과 클라이언트2의 요청을 처리할 때 사용하는 DB 커넥션이 같기 때문이다.
// 3) 클라이언트1
//    - 파일명을 255자가 넘어가게 입력한다.
//    - 예외가 발생하면 rollback()할 것ㅇ다ㅏ.
//    - 그러나 이미 이전에 insert 작업들이 클라이언트2가 commit하면서 테이블에 저장되었기 때문에
//    - rollback할 수 없다. 이것이 멀티 스레드에서 DB 커넥션을 공유할 때의 문제점이다.
//
//[해결책 I]
...
//    => 이때 첨부파일 2개를 입력한 후, 세 번째 입력할 때 예외가 발생하도록
//       긴 이름의 파일 명을 입력한다.

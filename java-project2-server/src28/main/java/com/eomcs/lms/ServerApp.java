...
//    => mvnrepository.com에서 log4j를 검색한다.
//    => build.gradle에 라이브러리 추가한다.
//    => 'gradle eclipse' 실행한다.
//    => eclipse 프로젝트 refresh한다.
// 2) Log4j 설정 파일을 준비한다.
...
//    => MybatisConfig.java에서 SqlSessionFactory 생성하는 메서드 안에 다음 코드 추가.
//       LogFactory.useLog4JLoggin();
//
// Log4J 설정법
// => CLASSPATH 루트 패키지에 log4j.properties 이름으로 설정 파일을 둔다.
// 1) 출력 등급 설정
// => log4j.rootLogger=DEBUG, ...
//    - rootLogger에 설정하면 자바의 모든 클래스에 대해 적용된다.
// => log4j.logger.com.eomcs.lms.service=ERROR
//    - com.eomcs.lms.service 패키지에 대해서는 ERROR 레벌을 적용한다.
//    - 즉, rootLogger에 기본 출력 등급을 설정하고, 
//      나머지 특정 패키지 전체나 특정 클래스에 대해 등급을 바꿀 때 사용한다.
// => 다음과 같이 등급을 조정할 수 있다.
//    rootLogger=DEBUG
//        com.eomcs.lms.dao=ERROR
//        com.eomcs.lms.service=FATAL
//        com.eomcs.lms.handler=INFO
//        기타 나머지를 따로 지정하지 않으면 rootLogger의 출력 등급을 적용한다.
// => 등급
...
//    INFO  : 애플리케이션의 주요 실행 정보 예) 어떤 IP의 클라이언트 접속 정보. 연결 종료.[단순정보]
...
//    - INFO 등급의 로그만 출력하도록 했으면, INFO 보다 큰 레벨(WARN, ERROR, FATAL)의
//      로그도 출력한다.
//
// 2) 출력 대상 지정
// => 어디로 출력할 것인지 지정한다.
...
// => log4j.appender.담당자명.layout=출력형식을 다룰 클래스(패키지명포함)
